...
void   Model_Switch_HMV ()
{
  ...
  if ( !_stricmp ("QUINCY1",soilmoddat ) )
      strcpy (ComModel,"CSVA00R4"); 

  if ( !_stricmp ("QUINCYD", soilmoddat ) )   
    strcpy (ComModel,"QDCSVA00");

  if ( !_stricmp ( "PBAIRG", soilmoddat ) )   
     strcpy (ComModel,"PBACSVA10"); 

  if ( !_stricmp ("PBWETG", soilmoddat ) )   
      strcpy (ComModel,"PBWCSVA10a"); 
  
  if ( !_stricmp ("BOULDERCW", soilmoddat ) )   
     strcpy (ComModel,"BCWCSVA00R0"); 

  if ( !_stricmp ("Manitou04", soilmoddat ) )    
